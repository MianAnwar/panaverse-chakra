...
import { GeneralQuaters } from '../../../constants/constants';
                        ...
                        return <Box key={index} my="5px" ml="10px">
                          ...
                          {outline.topicsCovered.map((topic, index) => {
                            return <Box key={index} ml="10px">
                              {topic.link != "" && <Text>{index + 1}. <Link color="blue.600" href={topic.link}>{topic.topic}</Link></Text>}
                              {topic.link == "" && topic.note == "" && <Text>{index + 1}. {topic.topic}</Text>}
                              {topic.note != "" && <chakra.text fontSize="small">{topic.note}</chakra.text>}
                            </Box>
                          })}
                        </Box>
                      })}
